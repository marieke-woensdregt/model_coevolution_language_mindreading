...
from context import gen_context_matrix
        ...
        if i == 0:
            ...
            for a in range(len(selected_hyps_per_agent)):
                ...
                if pragmatic_level_parent == 'literal' or pragmatic_level_parent == 'perspective-taking':
                    parent = Agent(perspective_hyps, lexicon_hyps, composite_log_prior, composite_log_prior, parent_perspective, sal_alpha, parent_lexicon, 'sample')
                elif pragmatic_level_parent == 'prag':
                    parent = PragmaticAgent(perspective_hyps, lexicon_hyps, composite_log_prior, composite_log_prior, parent_perspective, sal_alpha, parent_lexicon, learning_type, pragmatic_level_parent, pragmatic_level_learner, optimality_alpha, extra_error)
                if pragmatic_level_learner == 'literal' or pragmatic_level_learner == 'perspective-taking':
                    learner = Agent(perspective_hyps, lexicon_hyps, composite_log_prior, composite_log_prior, learner_perspective, sal_alpha, learner_lexicon, learning_type)
                elif pragmatic_level_learner == 'prag':
                    learner = PragmaticAgent(perspective_hyps, lexicon_hyps, composite_log_prior, composite_log_prior, learner_perspective, sal_alpha, learner_lexicon, learning_type, pragmatic_level_parent, pragmatic_level_learner, optimality_alpha, extra_error)
                context_matrix = gen_context_matrix('continuous', n_meanings, n_meanings, n_interactions)
                ca = calc_comm_acc(context_matrix, communication_type, ca_measure_type, n_interactions, n_utterances, parent, learner, n_meanings, n_signals, sal_alpha, production_error, parent.pragmatic_level, speaker_p_hyp=parent.perspective, speaker_type_hyp=parent.pragmatic_level)
