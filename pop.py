...
class Agent(object):
    ...
    def produce_data(self, n_meanings, n_signals, context_matrix, n_utterances, error):
        """
        :param n_meanings: The number of meanings
...
class PragmaticAgent(Agent):
    ...
    def __init__(self, perspective_hyps, lexicon_hyps, log_priors, log_posteriors, perspective, sal_alpha, lexicon, learning_type, speaker_type, listener_type, optimality_alpha):
        """
...
class Population(object):
    ...
    def create_pop(self):
        ...
        for i in range(self.size):
            ...
            if self.pragmatic_level == 'literal' or self.pragmatic_level == 'perspective-taking':
                ...
                agent = PragmaticAgent(self.perspective_hyps, self.lexicon_hyps, composite_log_priors_population, composite_log_priors_population, perspective, self.sal_alpha, lexicon, learning_type, self.pragmatic_level, self.pragmatic_level, self.optimality_alpha)
    ...
    def calc_ca_lex_n_context(self, ca_measure_type, n_interactions, parent, learner):
        ...
        parent_data = parent.produce_data(self.n_meanings, self.n_signals, context_matrix[0:n_parent_productions], self.n_utterances, self.error)
        ...
        for i in range(len(parent_data.topics)):
            for j in range(len(parent_data.topics[0])):
                ...
                counter += 1
        if ca_measure_type == 'comp_n_prod':
            learner_data = learner.produce_data(self.n_meanings, self.n_signals, context_matrix[n_parent_productions:], self.n_utterances, self.error)
    ...
    def calc_ca_pragmatic(self, ca_measure_type, n_interactions, parent, learner):
        if ca_measure_type != 'comp_only':
            raise ValueError("lex_n_perspective method of CA calculation only works when ca_measure_type == 'comp_only'")
        # Below the learner picks a perspective hypothesis from their posterior distribution to assign to their parent:
        winning_hyp_index = learner.pick_winning_hyp()
    ...
    def calc_comm_acc(self, communication_type, ca_measure_type, n_interactions, parent, learner):
        if communication_type == 'lex_only':
            ...
            mean_ca = self.calc_ca_pragmatic(ca_measure_type, n_interactions, parent, learner)
        # return ca_per_context, mean_ca
        return mean_ca
    ...
    # def pop_update(self, dataset_array_pickle_file_name, log_likelihood_pickle_file_name, recording, context_generation, helpful_contexts, n_meanings, n_signals, error, turnover_type, selection_type, selection_weighting, communication_type, ca_measure_type, n_interactions, teacher_type, perspectives_per_agent=None):
        ...
        for i in range(n_agents_to_be_replaced):
            ...
            if self.pragmatic_level == 'literal' or self.pragmatic_level == 'perspective-taking':
                ...
                new_agent = PragmaticAgent(self.perspective_hyps, self.lexicon_hyps, composite_log_priors, composite_log_priors, new_agent_perspective[0], self.sal_alpha, new_agent_lexicon, new_agent_learning_type[0], self.pragmatic_level, self.pragmatic_level, self.optimality_alpha)
    ...
    def create_pop(self):
        ...
        for i in range(self.size):
            ...
            if self.pragmatic_level_initial_pop == 'literal' or self.pragmatic_level_initial_pop == 'perspective-taking':
                ...
                agent = PragmaticAgent(self.perspective_hyps, self.lexicon_hyps, composite_log_priors_population, composite_log_priors_population, perspective, self.sal_alpha, lexicon, learning_type, self.pragmatic_level_parent_hyp, self.pragmatic_level_initial_pop, self.optimality_alpha)
        ...
        for i in range(n_agents_to_be_replaced):
            ...
            if i < n_agents_to_be_replaced - n_mutants:
                ...
                if self.pragmatic_level_mutants == 'literal' or self.pragmatic_level_mutants == 'perspective-taking':
                    ...
                    new_agent = PragmaticAgent(self.perspective_hyps, self.lexicon_hyps, composite_log_priors, composite_log_priors, new_agent_perspective[0], self.sal_alpha, new_agent_lexicon, new_agent_learning_type[0], self.pragmatic_level_mutants, self.pragmatic_level_mutants, self.optimality_alpha_mutants)
    ...
    # def pop_update(self, dataset_array_pickle_file_name, log_likelihood_pickle_file_name, recording, context_generation, helpful_contexts, n_meanings, n_signals, error, turnover_type, selection_type, selection_weighting, communication_type, ca_measure_type, n_interactions, teacher_type, perspectives_per_agent=None):
        ...
        for i in range(n_agents_to_be_replaced):
            ...
            if bio_parent.pragmatic_level == 'literal' or bio_parent.pragmatic_level == 'perspective-taking':
                ...
                new_agent = PragmaticAgent(self.perspective_hyps, self.lexicon_hyps, composite_log_priors, composite_log_priors, new_agent_perspective[0], self.sal_alpha, new_agent_lexicon, new_agent_learning_type[0], bio_parent.pragmatic_level, bio_parent.pragmatic_level, bio_parent.optimality_alpha)
