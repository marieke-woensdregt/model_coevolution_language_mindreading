...
def calc_ca_lex_n_perspective(context_matrix, ca_measure_type, parent, learner, speaker_p_hyp, sal_alpha, production_error):
    ...
    # speaker_p_hyp = perspective_hyps[learner_p_hyp_index]
...
def calc_ca_pragmatic(context_matrix, ca_measure_type, parent, learner, speaker_type, speaker_p_hyp, speaker_type_hyp, sal_alpha, production_error):
    if ca_measure_type != 'comp_only':
        raise ValueError("lex_n_perspective method of CA calculation only works when ca_measure_type == 'comp_only'")
    ca_per_context = np.zeros(len(context_matrix))
    for c in range(len(context_matrix)):
        ...
        l_rec_probs_according_to_speaker = parent.calc_pragmatic_listener(context, parent.lexicon.lexicon, parent.perspective, 'literal', production_error, sal_alpha)
        if speaker_type == 'literal':
            s_prod_probs = parent.calc_literal_speaker(context, parent.lexicon.lexicon, production_error, parent.perspective, sal_alpha)
        elif speaker_type == 'prag':
            s_prod_probs = parent.calc_pragmatic_speaker(l_rec_probs_according_to_speaker)
            parent_intention_reshaped = parent_intention.reshape(parent_intention.shape[0], -1)
            s_prod_probs_weighted_by_topic_probs = np.exp(np.add(np.log(s_prod_probs), np.log(parent_intention_reshaped)))
            s_prod_probs = np.divide(s_prod_probs_weighted_by_topic_probs, np.sum(s_prod_probs_weighted_by_topic_probs))
        log_s_prod_probs = np.log(s_prod_probs)
        l_rec_probs = learner.calc_pragmatic_listener(context, learner.lexicon.lexicon, speaker_p_hyp, speaker_type_hyp, production_error, sal_alpha)
        l_rec_probs_transposed = l_rec_probs.T
        log_l_rec_probs_transposed = np.log(l_rec_probs_transposed)
        log_multiplied_matrices = np.add(log_s_prod_probs, log_l_rec_probs_transposed)
        multiplied_matrices = np.exp(log_multiplied_matrices)
...
def calc_comm_acc(context_matrix, communication_type, ca_measure_type, n_interactions, n_utterances, parent, learner, n_meanings, n_signals, sal_alpha, production_error, speaker_type, speaker_p_hyp=None, speaker_type_hyp=None):
    if communication_type == 'lex_only':
        mean_ca = calc_ca_lex_only(ca_measure_type, parent, learner, n_meanings, n_signals, production_error)
    elif communication_type == 'lex_n_context':
        mean_ca = calc_ca_lex_n_context(context_matrix, ca_measure_type, n_interactions, parent, learner, n_utterances, n_meanings, n_signals, production_error)
    elif communication_type == 'lex_n_p':
        # ca_per_context, mean_ca = calc_ca_lex_n_perspective(n_interactions, parent, learner)
        mean_ca = calc_ca_lex_n_perspective(context_matrix, ca_measure_type, parent, learner, speaker_p_hyp, sal_alpha, production_error)
    elif communication_type == 'prag':
        # ca_per_context, mean_ca = calc_ca_pragmatic(n_interactions, parent, learner)
        mean_ca = calc_ca_pragmatic(context_matrix, ca_measure_type, parent, learner, speaker_type, speaker_p_hyp, speaker_type_hyp, sal_alpha, production_error)
    # return ca_per_context, mean_ca
    return mean_ca
